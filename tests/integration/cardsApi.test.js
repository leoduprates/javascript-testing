    ...
    it('should get 404 status error when no card with id sent', (done) => {
      request(server)
        ...
        .then((response) => {
          ...
          expect(response.body.message).toEqual('not found the card with id 61014b1134851b9203900c6e');
          done();
        });
    });
  ...
  describe('DELETE /api/cards', () => {
    ...
    it('should get 404 status error when no card with id sent', (done) => {
      request(server)
        ...
        .then((response) => {
          expect(response.statusCode).toBe(404);
          expect(response.body.message).toEqual('cannot delete card with id 61014b1134851b9203900c6e');
          done();
        });
    });
  });
